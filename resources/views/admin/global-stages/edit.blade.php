...
<div class="card">
    ...
    <div class="card-body">
        <form id="form" class="js-validate" action="{{ baseUrl('/global-stages/update/'.base64_encode($record->id)) }}" method="post">
            @csrf
            <div class="form-group row">
                <label class="col-sm-2 col-form-label">Title</label>
                <div class="js-form-message col-sm-10">
                    <input type="text" name="name" value="{{$record->name}}" required id="name" placeholder="Stage Profile Title" class="form-control">
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="custom-control custom-checkbox mt-3">
                  <input type="checkbox" class="custom-control-input row-checkbox" {{$record->visa_service_id != 0?'checked':''}} name="link_to_service" id="link_to_service" value="1">
                  <label class="custom-control-label" for="link_to_service">Yes</label>
                </div>
            </div>
            <div class="js-form-message form-group row visa_services" style="display: {{$record->visa_service_id != 0?'block':'none'}}">
                <label class="col-sm-2 col-form-label">Visa Service</label>
                <div class="col-sm-10">
                    <select name="visa_service_id">
                        <option value="">Select Service</option>
                        @foreach($services as $service)
                        <option value="{{ $service->service_id }}" {{$record->visa_service_id == $service->service_id?'selected':''}}>{{$service->Service($service->service_id)->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div id="addStagesContainer">
                  ...
                  <div class="item-row" data-index="{{$index}}">
                    ...
                    <div class="input-group-add-field">
                      ...
                      <input type="hidden" name="items[{{ $index }}][id]" value="{{$stage->id}}" />
                        <div class="col-md-9 js-form-message">
                          <input type="text" class="form-control mb-3 stage_name" value="{{$stage->name}}" name="items[{{$index}}][stage_name]" placeholder="Stage Name" aria-label="Stage Name">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-12">
                            <div class="sub-stages">
                              @foreach($stage->SubStages as $sub_stage)
                              @php 
                                $index2 = randomNumber(4);
                              @endphp
                              <div class="row sub-stage-row">
                                <input type="hidden" name="items[{{ $index }}][sub_stages][{{ $index2 }}][id]" value="{{$sub_stage->id}}" />
                                <div class="col-md-5">
                                    <div class="form-group">
                                      <input type="text" value="{{$sub_stage->name}}" required="" placeholder="Enter Sub Stage Name"
                                            name="items[{{ $index }}][sub_stages][{{ $index2 }}][name]" class="form-control sub_stage_name"></div>
                                </div>
                                <div class="col-md-5">
                                    <div class="form-group">
                                        <select required="" class="form-control sub_stage_type" name="items[{{ $index }}][sub_stages][{{ $index2 }}][stage_type]">
                                            <option value="">Select Type</option>
                                            <option data-type="fill-form" {{$sub_stage->stage_type == 'fill-form'?'selected':''}} value="fill-form">Fill Form</option>
                                            <option data-type="case-task" {{$sub_stage->stage_type == 'case-task'?'selected':''}} value="case-task">Case Task</option>
                                            <option data-type="case-document" {{$sub_stage->stage_type == 'case-document'?'selected':''}} value="case-document">Case Document</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="mt-3"><a href="javascript:;" onclick="removeSubStages(this)" class="text-danger"><i class="tio-clear"></i></a></div>
                                </div>
                              </div>
                              @endforeach
                            </div>
                      ...
                      <a class="js-delete-field input-group-add-field-delete" onclick="removeStage(this)" href="javascript:;" data-toggle="tooltip" data-placement="top" title="Remove item">
                        <i class="tio-clear"></i>
                      </a>
                    </div>
                    <!-- End Content -->
