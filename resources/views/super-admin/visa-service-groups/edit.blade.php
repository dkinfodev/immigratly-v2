...
<div class="visa_services">
  ...
  <div class="card">
    ...
    <div class="card-body">
      <form id="form" class="js-validate" action="{{ baseUrl('/visa-service-groups/update/'.base64_encode($record->id)) }}" method="post">
        @csrf
        <div class="form-group row">
          <label class="col-sm-2 col-form-label">Group Title</label>
          <div class="col-sm-10 js-form-message">
            <input type="text" name="group_title" id="group_title" placeholder="Enter Group Title" value="{{$record->group_title}}" class="form-control">
          </div>
        </div>
        <!-- End Input Group -->
        <div class="form-group row">
          ...
          <div class="col-sm-10 js-form-message">
            <textarea name="description" id="description" placeholder="Enter Description" class="form-control">{{$record->description}}</textarea>
          </div>
        </div>
        <div class="form-group row">
          ...
          <div class="col-sm-10 js-form-message">  
            ...
            @if(file_exists(public_path('uploads/visa-groups/'.$record->image)))
              <div class="visa_group_image mt-2">
                  <img src="{{ asset('/public/uploads/visa-groups/'.$record->image) }}" class="img-fluid" width="200" />
              </div>
            @endif
          </div>
        </div>
        <div class="form-group row">
          ...
          <div class="col-sm-10 js-form-message">
            <select name="program_type" id="program_type">
              ...
              <option {{$record->program_type == $program->id?'selected':''}} value="{{$program->id}}">{{$program->name}}</option>
              @endforeach
            </select>
          </div>
        </div>
        <div class="form-group row">
          @php
            ...
            if(!empty($record->VisaServices)){
              $visa_service_ids = $record->VisaServices->pluck("visa_service_id");
             
              if(!empty($visa_service_ids)){
                $visa_service_ids = $visa_service_ids->toArray();
              }
            }
          @endphp
          ...
          <div class="col-sm-10 js-form-message">
            <select name="visa_services[]" multiple id="visa_services">
              ...
              <option {{ (in_array($visa_service->unique_id,$visa_service_ids))?'selected':'' }} value="{{$visa_service->unique_id}}">{{$visa_service->name}}</option>
              @endforeach
            </select>
          </div>
        </div>
