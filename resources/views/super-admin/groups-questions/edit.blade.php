    ...
    <div class="card-body">
      <form id="form" class="js-validate" action="{{ baseUrl('/visa-services/eligibility-questions/'.base64_encode($visa_service->id).'/groups-questions/edit/'.base64_encode($record->id)) }}" method="post">
        ...
        <div class="js-form-message form-group row">
          ...
          <div class="col-sm-10">
            <textarea type="text" name="description" id="description" placeholder="Enter Group Description" class="form-control">{{$record->description}}</textarea>
          </div>
        </div>
        <div class="js-form-message form-group row">
        ...
        <div class="js-form-message form-group row">
          ...
          <div class="col-sm-10">
            <select id="selected_component" multiple placeholder="Select Compoent">
        ...
        <div class="js-form-message form-group row">
          <div class="col-md-12">
              <ul id="sortable" class="question-sort">
                @foreach($record->Components as $ques)
                <li data-ques-id="{{ $ques->component_id }}" data-sort="{{ $ques->sort_order }}" class="ui-state-default">
                  <span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
                  <input type="hidden" name="components[]" value="{{$ques->component_id}}" />
                  {{$ques->Component->component_title}}
                </li>
                @endforeach
              </ul>
          </div>
