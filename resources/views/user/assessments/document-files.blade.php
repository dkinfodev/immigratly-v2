    ...
    <div class="col-sm-auto mb-3 mt-3">
       @if($action != 'view')
        <div role="group">
           ...
           @endif
           <a class="btn btn-primary btn-sm"  href="javascript:;" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne"><i class="tio-upload-on-cloud mr-1"></i> Upload from Local</a>
        </div>
     </div>
     <?php
     ...
     ?>
      <div id="collapseOne" class="collapse" aria-labelledby="headingOne">
        <div class="card-body">
          <!-- Dropzone -->
            <div id="attachFilesLabel-{{ $id }}" data-id="{{$id}}" class="js-dropzone dropzone-custom custom-file-boxed"
            ...
            <div class="mt-2 text-right">
               <button type="button" onclick="uploadFiles()" class="btn btn-warning">Upload</button>
            </div>
            <!-- End Dropzone -->
        </div>
      </div>
               ...
               <tr>
                  ...
                  <td class="table-column-pl-0">
                     <?php
                        ...
                        $url = baseUrl('assessments/files/view-document/'.$doc->unique_id.'?url='.$doc_url.'&file_name='.$doc->FileDetail->file_name.'&folder_id='.$folder->unique_id."&assessment_id=".$assessment_id);
                     ?>
...
<script>
    ...
    // $('.dropzone-custom').each(function () {
      var id = "attachFilesLabel-{{$id}}";
      Dropzone.autoDiscover = false;
      dropzone = $.HSCore.components.HSDropzone.init('#' + id);
