...
class QuickEligibilityConroller extends Controller
{
    ...
    public function groupList()
    {
        ...
        $viewData['activeTab'] = "quick-eligibility";
        $program_types = ProgramTypes::get();
        $viewData['program_types'] = $program_types;
        return view('frontend.quick-eligibility.visa-groups',$viewData);
    } 
    ...
    {   
        ...
        $view = View::make('frontend.quick-eligibility.group-ajax-lists',$viewData);
    ...
    public function list($visa_group_id)
    {
        ...
        $viewData['activeTab'] = "quick-eligibility";
        return view('frontend.quick-eligibility.lists',$viewData);
    } 
    ...
    {   
        ...
        $viewData['records'] = $records;
        $view = View::make('frontend.quick-eligibility.ajax-list',$viewData);
        $contents = $view->render();
        $response['contents'] = $contents;
        $response['last_page'] = $records->lastPage();
        $response['current_page'] = $records->currentPage();
    ...
    public function eligibilityCheck($visa_service_id){
        ...
        $viewData['activeTab'] = "quick-eligibility";
        return view('frontend.quick-eligibility.quick-eligibility',$viewData);
    }
    public function eligibilityReport($report_id){
        ...
        $dependent_response  = array();
        if($record->dependent_response != ''){
            $dependent_response = json_decode($record->dependent_response,true);
        ...
        $viewData['dependent_response'] = $dependent_response;
        $viewData['final_questions'] = $final_questions;
        return view('frontend.quick-eligibility.score',$viewData);
    }
    public function fetchConditional(Request $request){
        ...
        $view = View::make('frontend.quick-eligibility.conditional-questions',$viewData);
        $contents = $view->render();
    ...
    public function fetchGroupConditional(Request $request){
        ...
        if(!empty($record)){
            ...
            $viewData['component'] = $component;
            $view = View::make('frontend.quick-eligibility.group-conditional-questions',$viewData);
            $contents = $view->render();
            $response['status'] = true;
            $response['contents'] = $contents;
        }else{
    ...
    public function saveEligibilityScore($visa_service_id,Request $request){
        ...
        $response['redirect_back'] = baseUrl("quick-eligibility/report/".$unique_id);
    ...
    public function allEligibility(){
        ...
        $viewData['activeTab'] = "quick-eligibility";
        return view('frontend.quick-eligibility.all-check',$viewData);        
    }
    ...
    {   
        ...
        if(count($records) == 0){
            ...
            $response['redirect_url'] = baseUrl("/quick-eligibility");
            ...
            $viewData['questions'] = EligibilityQuestions::where("visa_service_id",$visa_service->unique_id)->get();
            $viewData['pageTitle'] = "Eligibility Check of ".$visa_service->name;
            $view = View::make('frontend.quick-eligibility.eligibility-form',$viewData);
            $contents = $view->render();
            $response['status'] = true;
            $response['contents'] = $contents;
            $response['last_page'] = $records->lastPage();
    ...
    public function groupEligibilityCheck($visa_service_id){
        ...
        $view = View::make('frontend.quick-eligibility.group-form',$viewData);
        $group_form =  $view->render();
        
        $viewData['group_form'] = $group_form;
        $viewData['pageTitle'] = "Eligibility Check of ".$visa_service->name;
        $viewData['activeTab'] = "quick-eligibility";
        return view('frontend.quick-eligibility.group-eligibility-check',$viewData);
    }
    ...
    public function allGroupEligibilityCheck(){
        ...
        return view('frontend.quick-eligibility.all-group-eligibility',$viewData);
    }
        ...
        foreach($component_questions as $comp_id => $question_ids){
            ...
            foreach($question_ids as $key => $value){
                ...
                if(!empty($option)){       
                    ...
                    $comp_ques_score[$comp_id][$option->question_id]['option_value'] = $option->option_value;
...
;                       if($combination->behaviour == 'add'){
        ...
        // pre($comp_ques_score);
        $dependent_visa_service = VisaServices::where('dependent_visa_service',$visa_service_id)->get();
        $dependent_response = array();
        foreach($dependent_visa_service as $visa_service){
            $dependent_elg_questions = EligibilityQuestions::where("visa_service_id",$visa_service->unique_id)->get();
            $dependent_service = array();
            if(!empty($dependent_elg_questions)){
                // pre($dependent_elg_questions->toArray());
                
                $dep_ques_score = array();
                foreach($dependent_elg_questions as $elg_question){
                    $elgoptions = $elg_question->Options;
                    $dependent_component = DependentQuestionComponent::where("question_id",$elg_question->unique_id)->first();
                    if(!empty($dependent_component)){
                        // echo "COMP: ";
                        if(isset($comp_ques_score[$dependent_component->component_id][$dependent_component->linked_question_id])){
                            // echo "EEE<br>";
                            foreach($elgoptions as $opt){
                                $dep_score = $comp_ques_score[$dependent_component->component_id][$dependent_component->linked_question_id];
                                if($opt->option_value == $dep_score['option_value']){
                                    $temp['question'] = $elg_question->question; 
                                    $temp['option_value'] = $opt->option_label;
                                    $temp['option_score'] = $opt->score;
                                    $dep_ques_score[] = $temp;
                                }
                            }
                        }
                ...
                if(!empty($dep_ques_score)){
                    $dependent_service['service_name'] = $visa_service->name;
                    $dependent_service['questions_score'] = $dep_ques_score;
                }
                if(!empty($dependent_service)){
                    $dependent_response[] = $dependent_service;
                }
            }
        ...
        if(!empty($dependent_response)){
            $object->dependent_response = json_encode($dependent_response);
        }
        ...
        $download = '<a href="'.baseUrl('quick-eligibility/download-report/'.base64_encode($object->id)).'" class="btn btn-primary"><i class="tio-download"></i> Download Report</a>';
    ...
    public function downloadReport($id){
        ...
        $viewData['cutoff_points'] = $cutoff_points;
        // $view = View::make('frontend.quick-eligibility.group-report',$viewData);
        // $contents = $view->render();
        // echo $contents;
        $viewData['score'] = $report->score;
        $viewData['report'] = $report;
        $pdf_doc = \PDF::loadView('frontend.quick-eligibility.group-report', $viewData);
    ...
    public function userEligibilityHistory()
    {
        ...
        $program_types = ProgramTypes::get();
        $viewData['program_types'] = $program_types;
        return view('frontend.quick-eligibility.eligibility-history',$viewData);
    } 
    ...
    {   
        ...
        $viewData['records'] = $records;
        $view = View::make('frontend.quick-eligibility.history-ajax-lists',$viewData);
        $contents = $view->render();
        $response['contents'] = $contents;
        $response['last_page'] = $records->lastPage();
        $response['current_page'] = $records->currentPage();
    ...
    public function checkLanguageProficiency(Request $request){
        $question = EligibilityQuestions::where("unique_id",$request->input("question_id"))->first();
        ...
        $language_test = $request->input("language_test");

        $language_proficiency = LanguageProficiency::where("unique_id",$language_test)->first();
        // pre($language_proficiency);
        // exit;
        $clb_level = $language_proficiency->ClbLevels;
        $lng_scores['reading'] = $reading;
        $lng_scores['writing'] = $writing;
        $lng_scores['listening'] = $listening;
        $lng_scores['speaking'] = $speaking;
        ...
        foreach($options as $option){
            if($option->option_value == $clb_score){
                if($question->language_type == 'first_official'){
                    if($option->language_proficiency_id == $language_test){
        ...
        $response['option_type'] = $question->option_type;
        return response()->json($response);
    }
}
